...
handle_select_switch
  ...
  if gamestate = IN_PROGRESS then gosub clear_sounds bank1 : gosub stop_game bank2 : gamestate = MODE_SELECT
...
handle_active_game
  ...
  rem Restart Game
  if switchreset then gosub clear_sounds bank1 : gosub start_game bank2 : return thisbank
  ...
  if switchrightb then gosub set_p0_player_movement bank1 else gosub set_p0_cpu_movement bank1
